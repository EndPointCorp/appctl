...
acme.Kiosk.prototype.initOnce = function() {
  ...
  // don't subscribe ambient mode until we are ready to start tours
  setTimeout(function() {
    interactionTopic.subscribe(ambient.handleInteractionMessage.bind(ambient));
  }, 10000);
};
...
var interactionTopic = new ROSLIB.Topic({
  ros: portalRosKiosk,
  name: '/portal_occupancy/interaction/inactive_duration',
  messageType: 'std_msgs/Duration'
});
...
joystickTopic.subscribe(function(twist) {
  ...
  fanControl.handleNavTwist(twist);
});
...
.done(function( data ) {
  console.log("Kiosk - connected to indetify.py");
  initROSLogging("kiosk", data);
})
.fail(function(){
  console.log("Kiosk - failed to connect to indetify.py");
  initROSLogging("kiosk", "--");
});
