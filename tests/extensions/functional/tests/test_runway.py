...
class TestRunway(TestBase):
    ...
    def setup_method(self, method):
        print "setup method %s called" % self.__class__.__name__
    ...
    @staticmethod
    def prepare_poi(config, browser):
        """
        ...
        poi_button = browser.find_element_by_id("acme-poi-button")
        poi_button.click()

        # it takes some time until they appear, wait explicitly
        def tester(browser_ref):
            poi_container = browser.find_element_by_id("acme-points-of-interest")
    ...
    def test_runway_points_of_interest(self):
        ...
        """
        self.prepare_poi(self.config, self.browser)
