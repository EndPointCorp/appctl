...
class TestBaseTwoBrowsersROS(TestBase):
    ...
    def test_ros_points_of_interest_visit_synchronization(self):
        ...
        c = 0
        # have to retrieve the pointer to POIs every iteration
        # otherwise sometimes getting chrome browser error due to stale element
        # not attached to the document
        for index in range(len(points)):
            points_ref = sorted(poi_container.find_elements_by_class_name("widget-runway-card-button"))
            point = points_ref[index]
            action.move_to_element(point).click().perform()
            time.sleep(5)
