...
class TestBase(object):
    ...
    def click(self, finder_value, finder, browser=None):
        """
        ...
        curr_browser.execute_script(event_script)

    def click_zoom_in(self, browser=None):
        """
        ...
        """
        curr_browser = browser if browser else self.browser
        old_url = curr_browser.current_url
        self.click("widget-zoom-in", finder="by_class", browser=curr_browser)
        self.wait_for_url_change(old_url, browser=curr_browser)

    def click_zoom_out(self, browser=None):
        """
        ...
        curr_browser = browser if browser else self.browser
        old_url = curr_browser.current_url
        self.click("widget-zoom-out", finder="by_class", browser=curr_browser)
        self.wait_for_url_change(old_url, browser=curr_browser)

    def wait_for_url_change(self, old_value, browser=None, interval=1, max_wait_time=20):
        """
        ...
        while (start + max_wait_time > time.time()):
            if old_value != curr_browser.current_url:
                return
            time.sleep(interval)